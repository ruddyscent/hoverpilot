...
class HoverEnvTests(unittest.TestCase):
    ...
    def test_wait_for_next_episode_detects_lost_components_recovery_after_boundary_termination(self):
        client = StubRFLinkClient([
            self._state(),
            self._state(m_currentPhysicsTime_SEC=10.2, m_aircraftPositionX_MTR=11.0, m_hasLostComponents=0.0),
            self._state(m_currentPhysicsTime_SEC=10.4, m_hasLostComponents=1.0),
            self._state(m_currentPhysicsTime_SEC=10.6, m_hasLostComponents=0.0),
        ])
        env = HoverPilotHoverEnv(
            host="127.0.0.1",
            port=18083,
        ...
        self.assertEqual(observation.shape, (12,))
        self.assertEqual(next_info["episode_start_reason"], "crash_reset")
        env.close()

    def test_wait_for_next_episode_detects_controller_reactivation(self):
        client = StubRFLinkClient([
            self._state(),
            self._state(m_currentPhysicsTime_SEC=10.2, m_aircraftPositionX_MTR=11.0, m_flightAxisControllerIsActive=0.0),
            self._state(m_currentPhysicsTime_SEC=10.4, m_flightAxisControllerIsActive=1.0),
        ])
        env = HoverPilotHoverEnv(
            host="127.0.0.1",
            port=18083,
        ...
        self.assertTrue(started)
        self.assertEqual(observation.shape, (12,))
        self.assertEqual(next_info["episode_start_reason"], "controller_reactivated")
        env.close()


    def test_wait_for_next_episode_detects_crash_signature_recovery(self):
        client = StubRFLinkClient([
            self._state(),
            self._state(m_currentPhysicsTime_SEC=10.2, m_aircraftPositionX_MTR=11.0),
            self._state(
                m_currentPhysicsTime_SEC=10.4,
                m_hasLostComponents=1.0,
                m_anEngineIsRunning=0.0,
                m_isTouchingGround=1.0,
            ),
            self._state(
